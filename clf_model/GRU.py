...
class GRUModel(nn.Module):
    def __init__(self, num_class, embedding_dim, hidden_dim, max_sequence_length=32):
        # stacked Bi-GRU
        super().__init__()
        self.GRUs = nn.GRU(embedding_dim, hidden_dim, num_layers=2, batch_first=True, dropout=0.3, bidirectional=True)
        if self.GRUs.bidirectional:
